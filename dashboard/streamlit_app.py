...
def get_counts():
    e = get_engine()
    observation_count = pd.read_sql(queries.VAR_COUNT_TABLE.format(table="weather_observations"), e).iloc[0][0]
    metadata_count = pd.read_sql(queries.VAR_COUNT_TABLE.format(table="fetch_metadata"), e).iloc[0][0]
...
def main():
    st.set_page_config(
        page_title="Meteo ETL Dashboard",
        layout="wide"
    )
