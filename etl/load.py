...
def load_observation_rows(
    weather_records: Sequence[WeatherRecord], fetch_id: uuid.UUID, session: Session
):
    """
    ...
    """
    exclude_columns = set(WeatherRecord.model_fields.keys()) - set(
    ...
    try:
        ...
        logger.exception(
            "Failed loading observations rows (fetch=%s, rows=%d)",
            fetch_id,
            len(weather_records),
        )
...
def insert_fetch_metadata(url: str, params: dict, session: Session) -> uuid.UUID:
    ...
    stmt = (
        ...
        .values(
            request_url=url, request_params=params, request_timestamp=datetime.utcnow()
        )
