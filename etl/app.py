...
from typing import Sequence, Dict, Tuple, NamedTuple
...
def etl(long: float, lat: float, source: SourceName, session_factory: sessionmaker, **extra_params) -> uuid.UUID:
    """
    ...
    if error_occurred:
        logger.exception(error_msg, extra=dict(long=long, lat=lat, source=source, **extra_params))
        raise ETLError(f"Error occurred fetching {fetch_id}", fetch_id=fetch_id) from error_occurred
