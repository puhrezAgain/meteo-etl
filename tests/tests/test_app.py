...
class TestETApp:
    ...
    def test_et_errors(self, error, monkeypatch):
        monkeypatch.setattr(
            "etl.sources.MeteoSource.run_transform",  
            lambda *args: _raise(error))
        
        with pytest.raises(ETLError):
            et(5.0, 3.0, SourceName.METEO)

    @pytest.mark.integration
    def test_etl_success(self, override_meteo_api, monkeypatch, db_session_maker, db_session, weather_records):
